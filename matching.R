...
take_snippet <- function(start = 370, end = 380){ 
  if(end - start < 10){
    stop("Recording snippet isn't long enough, try again")
  }
  if(any(diff(song_data[c(start:end), 1]) != 0)){
    stop("windows are not from the same song")
  }
  if(start > nrow(song_data) | end > nrow(song_data)){
    stop("wrong input")
  }
  snippet_data <- song_data[c(start:end), -c(1,2)]
...
generate_next_search_matrix <- function(match_index, search_matrix){ 
  nextWinInfo <- search_matrix[match_index, 1:2]
  nextWinInfo[,2] <- nextWinInfo[,2]+1
  nextWinInd <- match(data.frame(t(nextWinInfo)), data.frame(t(song_data[,1:2]))) #check id-window pair exists in song_data
...
song_data <- dbGetQuery(conn, "select * from song_data;") #extract song data (signature, window center and song id) from server
search_matrix <- song_data #initial matrix for matching is equal to entire song database

match_signature <- function(snippet,threshold = 0.01, N = 2) { # N is the number of matches allowed to return
  frequency_id <- c()
  for (i in 1:nrow(snippet)) {
    distance <- matrix(dist(rbind(snippet[i, ], search_matrix[, -c(1, 2)]), method = "euclidean"))[1:nrow(search_matrix), 1]
    match_index <- which(distance <= threshold) #identify the indices where distance is less than threshold
    if(all(is.na(match_index))){
      return(NULL)
    }
    frequency_id <- c(frequency_id, search_matrix[match_index, 1])  #store song_id generated by each loop in order to count the number of time the song matches
    next_index <- generate_next_search_matrix(match_index, search_matrix) #if succeeding id-window pair exists in song database, return next index of window
